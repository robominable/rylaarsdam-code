...
task usercontrol()
{
	...
	while (true)
	{
		...
		{
			freezeLift = false;
			if (autoLiftPrimed == true) {autoLift = true; resetLiftPID();}
		}

		//Auto Lift Control
		if (autoLiftPrimed == true)
		{
			if (SensorValue(LiftPoten) < 3400)
			{
				autoLiftPrimed = false;
				liftPotenTarget = 3400;
				autoLift = true;
...
void resetArmPID()
{
	...
	//*********************************************************************
	TpRArm = 0; //TUNABLE: [Starting] Power of right launcher
	errorRArm = 0; //Error of right launcher

	iRArm = 0; //Integral of right launcher

	dRArm = 0; //Derivative of right launcher
	errorRArmPrevious = 0;

	editRArm = 0;

	RarmPotenTarget = SensorValue(RarmPoten);
}
