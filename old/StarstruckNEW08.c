...
#pragma config(Motor,  port5,           ALift,         tmotorVex393_MC29, openLoop, reversed)
...
task usercontrol()
{
	...
	while (true)
	{
		...
		if (autoLift == false && freezeLift == false)
		{
			...
			motor[LBLift] = ((vexRT[Btn5D] - vexRT[Btn5U]) * 127) + ((vexRT[Btn7U] - vexRT[Btn7D]) * 40 );
		}

		if (autoLift == false && freezeLift == false && SensorValue(LiftPoten) > 3460 && IgnoreLiftFloor == false)
		{
			liftPotenTarget = 3400;
			autoLiftPrimed = true;
			autoLift = true;
		...
		if (SensorValue(LiftPoten) <= 3460 && autoLift == true)
		{
			autoLift = false;
			autoLiftPrimed = false;
			motor[LBLift] = 0;
