...
int liftPotenVal = 2978;
int liftPotenTarget = 2978;

float KpArm = 0.000002;
float KiArm = 0;
float KdArm = 0.2;

float KpLift = 0.00000000000002;
float KiLift = 0;
float KdLift = 0.2;
...
task usercontrol()
{
	...
	while (true)
	{
		...
		//{
			...
			TpArm = TpArm + editArm; // MotorPower = PreviousMotorPower + Edit
		//}
		...
		if (vexRT[Btn8L]) { liftPotenTarget = 4095; } //full down
		else if (vexRT[Btn8U]) { liftPotenTarget = 2978; } //down
		else if (vexRT[Btn8R]) { liftPotenTarget = 1528; } //up
	}
}
